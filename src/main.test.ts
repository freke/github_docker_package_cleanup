...
const mockInput = (inputs: Record<string, string | boolean>) => {
  ...
  (core.getBooleanInput as jest.Mock).mockImplementation((name) => inputs[name] || false);
};

// Helper function to mock API responses
const mockApiResponse = (data: any[]) => {
  (mockOctokit.paginate as jest.Mock).mockResolvedValue(data);
};
...
describe('run', () => {
  ...
  it('should delete old non-semver versions', async () => {
    ...
    expect(mockOctokit.rest.packages.deletePackageVersionForAuthenticatedUser).toHaveBeenCalledTimes(1);
    expect(mockOctokit.rest.packages.deletePackageVersionForAuthenticatedUser).toHaveBeenCalledWith({
  ...
  it('should not delete versions with "latest" tag', async () => {
    ...
    // Expect no deletions
    expect(mockOctokit.rest.packages.deletePackageVersionForAuthenticatedUser).not.toHaveBeenCalled();
  ...
  it('should handle invalid regex patterns in keep input', async () => {
    ...
    expect(core.warning).toHaveBeenCalledWith('Invalid regex pattern ignored: [invalid-regex');
  ...
  it('should perform dry run and not delete anything', async () => {
    ...
    expect(core.info).toHaveBeenCalledWith('[DRY-RUN] Would delete version 1 (Tags: v1.0.0-old)');
    expect(mockOctokit.rest.packages.deletePackageVersionForAuthenticatedUser).not.toHaveBeenCalled();
  ...
  it('should use deletePackageVersionForOrg when org input is provided', async () => {
    ...
    expect(mockOctokit.rest.packages.deletePackageVersionForOrg).toHaveBeenCalledTimes(1);
  ...
  it('should handle API errors gracefully', async () => {
    ...
    (mockOctokit.paginate as jest.Mock).mockRejectedValue(new Error(errorMessage));
